...
public class CustomerResearchFundAction extends Action {
	...
	public String perform(HttpServletRequest request) {
		...
		try {
			...
			FundBean fund = DAOFund.getFundByName(fundName);
			if (fund == null) {
			...
			// prices.get(i).setPrice(prices.get(i).getPrice() / 100);
			// }
			/*
			 * for (FundPriceBean fundPriceBean : prices) {
			 * fundPriceBean.setPriceDate(
			 * converStringDateFormat(fundPriceBean.getPriceDate())); }
			 */
